 ...
 */
@Component
public class InventoryServiceClient implements ServiceClientPort {
    ...
    public ServiceName getServiceName() {
        return ServiceName.INVENTORY;
    }

    @Override
    public NotifyResponse notify(ServiceName serviceName, NotifyRequest request) {
        if (serviceName != ServiceName.INVENTORY) {
            throw new IllegalArgumentException("This client only handles INVENTORY service");
        }
        return notify(baseUrl + "/api/v1/inventory/notify", request);
    }
    ...
    public RollbackResponse rollback(ServiceName serviceName, RollbackRequest request) {
        ...
        return rollback(baseUrl + "/api/v1/inventory/rollback", request);
    }

    @Override
    public NotifyResponse notify(String notifyUrl, NotifyRequest request) {
        log.info("Calling inventory notify: txId={}, url={}", request.txId(), notifyUrl);
        try {
            NotifyResponse response = restTemplate.postForObject(notifyUrl, request, NotifyResponse.class);
            log.info("Inventory notify response: txId={}, success={}", request.txId(), 
                    response != null && response.success());
            return response;
        } catch (Exception e) {
            log.error("Inventory notify failed: txId={}, error={}", request.txId(), e.getMessage(), e);
    ...
    public RollbackResponse rollback(String rollbackUrl, RollbackRequest request) {
        log.info("Calling inventory rollback: txId={}, url={}", request.txId(), rollbackUrl);
        try {
            RollbackResponse response = restTemplate.postForObject(rollbackUrl, request, RollbackResponse.class);
            log.info("Inventory rollback response: txId={}, success={}", request.txId(), 
                    response != null && response.success());
            return response;
        } catch (Exception e) {
            log.error("Inventory rollback failed: txId={}, error={}", request.txId(), e.getMessage(), e);
